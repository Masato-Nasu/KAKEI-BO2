const API_KEY_STORAGE_KEY = 'kakei-bo2-openai-api-key-v1';
...
function buildJsonFileName() {
  ...
  return `kakei-bo2-${safeLabel}-latest.json`;
}
...
function buildCsvFileName() {
  ...
  return `kakei-bo2-${yyyy}${mm}${dd}-${hh}${mi}${ss}.csv`;
}
...
saveBtn.addEventListener('click', async () => {
  ...
  try {
    const storedImageDataUrls = await buildStoredImageDataUrls();
    const storedImageDataUrl = storedImageDataUrls[0] || previewDataUrl || '';
    ...
    const entries = loadEntries();
    entries.push(entry);
    const saveResult = saveEntries(entries);
    renderHistory();
    resetForm();
    resetImage();
    statusEl.textContent = saveResult?.quotaFallback
  ...
  importJsonInput.addEventListener('change', async (event) => {
    ...
    try {
      ...
      const imageNote = result.quotaFallback
        ...
        : '';
      const message = `JSONを読み込みました。追加 ${result.addedCount}件 / 重複スキップ ${result.duplicateCount}件 / 合計 ${result.totalCount}件${imageNote}`;
